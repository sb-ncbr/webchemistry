...
function ValidatorErrorsModel(result) {
    ...
    _.forEach(result.Models, function (m) {
        var transformed;
        if (m.errorCount > 0) {
            transformed = transformErrors(m.Errors, m.ModelName);
            modelEntries.push({
                tag: m.ModelName + "err",
            ...
            _.forEach(transformed, function (e) {
        ...
        if (m.warningCount > 0) {
            transformed = transformWarnings(m.Warnings);
            modelEntries.push({
                tag: m.ModelName + "wrr",
    ...
    _.forEach(modelEntries, function (e) { self.entries.push(e); });
...
function createErrorModelVisualization(sid, modelName) {
    ...
    if (errorDetailsModelCache[cacheKey]) {
        ...
        xhr = $.ajax({
            ...
            data: { model: modelName, sid: sid, type: "notanalyzedjson" },
            cache: false,
            dataType: 'json'
        })
