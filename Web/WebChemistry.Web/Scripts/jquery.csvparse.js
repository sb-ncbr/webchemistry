...
(function ($) {
    ...
    function Parser(config) {
        ...
        _config = {
            ...
            step: config.step,
            forceStringFields: config.forceStringFields
        };

        var forceStringFields = _config.forceStringFields;
        ...
        function validConfig(config) {
            ...
            if (typeof config.forceStringFields !== 'object')
                config.forceStringFields = _defaultConfig.forceStringFields;
        ...
        function saveValue() {
            if (_config.header) {
                if (_state.lineNum == 1 && _invocations == 1)
                    ...
                    if (fieldName) {
                        if (_config.dynamicTyping && !forceStringFields[fieldName]) {
                            currentRow[fieldName] = tryParseFloat(_state.fieldVal);
                        } else {
                ...
                if (_config.dynamicTyping) {
                    _state.parsed[_state.parsed.length - 1].push(tryParseFloat(_state.fieldVal))
        ...
        function tryParseFloat(num) {
            ...
            var t = +num;
            return isNaN(t) /*&& !_regex.empty.test(num)*/ ? num : t;
        }
