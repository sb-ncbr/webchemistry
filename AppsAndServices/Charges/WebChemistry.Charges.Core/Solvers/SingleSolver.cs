namespace WebChemistry.Charges.Core.Solvers
{
    ...
    class SingleSolver : MatrixSolver
    {
            ...
            out HashSet<string> warnings)
        {
            ...
            var diagonal = new List<float>(atoms.Count);
            var resultVector = new List<float>(atoms.Count);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                float param = (float)(parentSet.ABFactor * value.B);
                diagonal.Add(param);

                param = (float)(-parentSet.ABFactor * value.A);
                resultVector.Add(param);
            }

            resultVector.Add((float)totalCharge);
            ...
            for (int i = 0; i < count - 1; i++)
            {
                ...
                for (int j = i + 1; j < count; j++)
                {
                    ...
                    float param = (float)(kappa / d);
                    eemMatrix[i, j] = param;
                    eemMatrix[j, i] = param;
                }
            }

            b[count] = (float)-totalCharge;
